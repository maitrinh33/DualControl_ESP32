...
struct PacketData {
  ...
  byte yAxisValue;
  byte zAxisValue;
  byte command; // NEW: Unified movement command (0–10)
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
  ...
  if (len != sizeof(receiverData)) {
    Serial.println("⚠️ Packet size mismatch - discarded");
    return;
  }

  memcpy(&receiverData, incomingData, sizeof(receiverData));
  lastRecvTime = millis();
  signalLost = false; // reset signal lost status
  digitalWrite(LED_PIN, HIGH); // Signal received
...
void loop() {
  ...
  if (lastCommandType != -1 && now - lastRecvTime > SIGNAL_TIMEOUT) {
    lastCommandType = -1; // allow new source
  }
