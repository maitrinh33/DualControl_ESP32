...
void setupWiFi() {
  ...
  if (!wifiManager.autoConnect("ESP32-Car", "12345678")) {
...
void initEspNow() {
  ...
  peerInfo.channel = 1;  // optional; you can get real channel with WiFi.channel()
...
/*=========== SEND CAR'S COMMANDS ===========*/
// Gửi lệnh điều khiển remote với command kiểu số
...
void onCarInputWebSocketEvent(AsyncWebSocket *server,
  ...
  switch (type) {
    ...
    case WS_EVT_DISCONNECT:
      ...
      // Khi ngắt kết nối, gửi lệnh STOP
      sendCarCommands(static_cast<command>(0));  // valid cast
...
void sendCameraFrame() {
  ...
  if (!fb) {
    ...
    // Only reinitialize up to maxRetries
  ...
  cameraRetryCount = 0;  // Reset counter on success
...
void setupCameraStreaming() {
  const int intervalMs = 100;  // ~10 FPS
...
void setup() {
  ...
  setupCamera();
  initEspNow();
  sendCarCommands(static_cast<command>(0));  // gửi lệnh STOP ban đầu
